...
int ans[MAXM+10];
int state[MAXN+10]; //0:haven't been in the stack; 1: in the stack; 2:has popped out;
int flag[MAXN+10];
int getFlag(int x){
...
void dfs(int now, int pa){ // now：正在搜的点， pa：parent节点
	...
	for(auto q: querys[now]){ //对于所有挂在这个节点上的询问
		...
		if(state[q.other]==1){ //如果另一个点的状态也是1
			ans[q.index] = q.other; //那么它们的最近公共祖先就是另一个店
		}else if(state[q.other]==2){ // 如果另一个点的状态是2
			ans[q.index] = getFlag(q.other); //那么它们的最近公共祖先就是另一个点所在的并查集的flag
		}
	}
